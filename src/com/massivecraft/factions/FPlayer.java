...
public class FPlayer extends PlayerEntity implements EconomyParticipator {
	...
	public FPlayer() {
		...
		this.autoSafeZoneEnabled = false;
		this.autoWarZoneEnabled = false;
		this.loginPvpDisabled = (Conf.noPVPDamageToOthersForXSecondsAfterLogin > 0) ? true
				: false;
		this.deleteMe = false;
		this.powerBoost = 0.0;
	...
	public String getChatTag() {
		...
		return String.format(Conf.chatTagFormat,
	...
	public double getPowerMin() {
		 return getPowerLossOnDeath() * 2.5 + this.powerBoost;
	}
	...
	public void onDeath() {
		this.updatePower();
		this.alterPower(getPowerLossOnDeath());
	}
	
	public double getPowerLossOnDeath() {
		  Faction faction = getFaction();
		  double n = 1D;
		  if (faction != null && (n = faction.getFPlayers().size()) > 0) {
	...
	public boolean canClaimForFaction(Faction forFaction) {
		...
		if (this.isAdminBypassing()
				|| (forFaction == this.getFaction() && this.getRole()
						.isAtLeast(Role.MODERATOR))
				|| (forFaction.isSafeZone() && Permission.MANAGE_SAFE_ZONE
						.has(getPlayer()))
				|| (forFaction.isWarZone() && Permission.MANAGE_WAR_ZONE
						.has(getPlayer()))) {
			return true;
		}
	...
	public boolean canClaimForFactionAtLocation(Faction forFaction,
		...
		if (Conf.worldGuardChecking
			...
			return true;
				...
				&& Permission.MANAGE_SAFE_ZONE.has(getPlayer())) {
			return true;
		} else if (forFaction.isWarZone()
				&& Permission.MANAGE_WAR_ZONE.has(getPlayer())) {
			return true;
		} else if (myFaction != forFaction) {
			error = P.p.txt.parse("<b>You can't claim land for <h>%s<b>.",
					forFaction.describeTo(this));
			...
			error = P.p.txt.parse("<b>You must be <h>%s<b> to claim land.",
			...
			error = P.p.txt
				...
				&& ownedLand >= Conf.claimedLandsMax && forFaction.isNormal()) {
			error = P.p.txt
					.parse("<b>Limit reached. You can't claim more land!");
		} else if (currentFaction.getRelationTo(forFaction) == Relation.ALLY) {
			error = P.p.txt
					.parse("<b>You can't claim the land of your allies.");
			...
			if (myFaction.isPeaceful()) {
				error = P.p.txt
						.parse("%s<i> owns this land. Your faction is peaceful, so you cannot claim land from other factions.",
								currentFaction.getTag(this));
			} else if (currentFaction.isPeaceful()) {
				error = P.p.txt
						.parse("%s<i> owns this land, and is a peaceful faction. You cannot claim land from them.",
								currentFaction.getTag(this));
	...
	public boolean attemptClaim(Player me, Faction forFaction,
			Location location, boolean notifyFailure) {
		...
		// if economy is enabled and they're not on the bypass list, make sure
		// they can pay
		boolean mustPay = Econ.shouldBeUsed() && !this.isAdminBypassing()
				&& !forFaction.isSafeZone() && !forFaction.isWarZone();
		double cost = 0.0;
		...
		if (LWCFeatures.getEnabled() && forFaction.isNormal()
				&& Conf.onCaptureResetLwcLocks) {
			LWCFeatures.clearOtherChests(flocation, this.getFaction());
		}

		// If not bypassing, make sure there is no non-members in chunk!
		if (!Permission.BYPASS.has(me)) {
			Chunk chunk = me.getLocation().getChunk();

			List<FPlayer> playersInChunk = new ArrayList<FPlayer>();

			for (Entity entity : chunk.getEntities()) {
				if ((entity instanceof Player)) {
					playersInChunk.add((FPlayer) FPlayers.i
							.get((Player) entity));
				}
			}

			for (FPlayer otherPlayer : playersInChunk) {
				if (!otherPlayer.getRelationTo(fme).isMember()) {
					msg("<b>Can't claim chunk because non-faction member in chunk!",
							new Object[0]);
					return false;
				}
			}
	...
	public boolean shouldBeSaved() {
		if (!this.hasFaction()
				&& (this.getPowerRounded() == this.getPowerMaxRounded() || this
